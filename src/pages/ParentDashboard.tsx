...
export default function ParentDashboard() {
  ...
  const checkRoleAndFetch = async () => {
    ...
    try {
      ...
      const {
        data: studentData
      } = await supabase.from('students').select('id').eq('user_id', user.id).maybeSingle();
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const loadedSettings = profileData?.header_settings && typeof profileData.header_settings === 'object' ? profileData.header_settings as any : {};
      const finalSettings = {
      ...
      const {
        data: studentsData
      } = await supabase.from('students').select('*').eq('parent_id', user.id);
      ...
      if (studentIds.length > 0) {
        ...
        const {
          data: completedData
        } = await supabase.from('submissions').select('id').gte('submitted_at', todayStart.toISOString()).in('student_id', studentIds);
      ...
      if (studentIds.length > 0) {
        const {
          data: overdueData,
          error: overdueError
        } = await supabase.from('assignments').select('id, curriculum_item_id, curriculum_items!inner(course_id, courses!inner(student_id))').eq('status', 'assigned').lt('due_at', new Date().toISOString()).in('curriculum_items.courses.student_id', studentIds);
        if (overdueError) {
  ...
  return <PomodoroProvider>
      <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5">
        ...
        {headerSettings && mockStudentForHeader && <CustomizableHeader student={mockStudentForHeader} settings={headerSettings} onSaveSettings={saveHeaderSettings} onSignOut={handleSignOut} onDemoCelebration={() => setShowConfetti(true)} />}
      ...
      <div className="container mx-auto p-4 md:p-8">
        ...
        <Tabs defaultValue="overview" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="smart-schedule">4</TabsTrigger>
          ...
          <TabsContent value="focus-intelligence" className="space-y-4">
            {students.length === 0 ? <Card>
                ...
                {students.map(student => <div key={student.id} className="space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                {students.length === 0 ? <div className="text-center py-12">
                    ...
                    {students.map(student => <Card key={student.id} className="hover:border-primary transition-colors">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0">
                          <div className="flex items-center gap-4 flex-1 cursor-pointer" onClick={() => navigate(`/student/${student.id}`)}>
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                {students.length === 0 ? <div className="text-center py-12">
                  ...
                  </div> : students.map(student => <Card key={student.id}>
                      <CardHeader>
      ...
      <EditStudentDialog student={editingStudent} open={!!editingStudent} onOpenChange={open => !open && setEditingStudent(null)} onStudentUpdated={fetchDashboardData} />

      <DeleteStudentDialog student={deletingStudent} open={!!deletingStudent} onOpenChange={open => !open && setDeletingStudent(null)} onStudentDeleted={fetchDashboardData} />
    </div>
    </PomodoroProvider>;
}