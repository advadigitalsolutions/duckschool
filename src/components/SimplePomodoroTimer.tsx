...
export function SimplePomodoroTimer({ studentId, compact = false }: SimplePomodoroTimerProps) {
  const {
    timeLeft,
    isRunning,
    isBreak,
  ...
  return (
    <div className={cn(
      ...
      <div className="relative">
        <svg className={containerSize} style={{ transform: 'rotate(-90deg)' }}>
          {/* Background circle */}
          <circle
            cx={compact ? 64 : 160}
            cy={compact ? 64 : 160}
      ...
      {!compact && (
        <div className="flex gap-4">
          <Button
            ...
            size="lg"
            className={cn(
              "gap-2 min-w-[160px] shadow-lg transition-all duration-200",
              isRunning && "shadow-primary/50"
            )}
          >
            {isRunning ? (
              <>
